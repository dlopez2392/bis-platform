...
--
-- Expected, allowed differences (plan section 1): extension_version(info);
-- event_trigger rows that are Supabase platform triggers; bucket settings.
-- Everything else must match. The CI project is pushed byte-exact from the
-- migration files, so it is the reference for "what the files say".
--
-- Two departures from the plan's text, both so that equal catalogues produce
-- equal digests:
--   - ACLs (schema_acl, function, default_acl) are compared as their items
--     SORTED, not as aclitem[]::text. The array's order is the order the
--     grants happened in, which differs between a project granted by the
--     platform and one granted by bootstrap/ci-project.sql, while the
--     privileges are identical. NULL (never touched) stays 'default'; an empty
--     array (everything revoked) is ''.
--   - The digest orders by key AND def, so two items with the same key (an
--     enum name in two schemas) cannot swap places between runs.
with items(kind, key, def) as (
  select 'extension', e.extname::text, null::text from pg_extension e
  union all
  select 'extension_version(info)', e.extname::text, e.extversion from pg_extension e
  union all
  select 'schema_acl', n.nspname::text,
         case when n.nspacl is null then 'default'
              else coalesce((select string_agg(a::text, ',' order by a::text) from unnest(n.nspacl) a), '') end
    from pg_namespace n where n.nspname in ('public', 'app', 'storage')
  union all
  ...
  select 'function', n.nspname || '.' || p.proname || '(' || pg_get_function_identity_arguments(p.oid) || ')',
         concat_ws(' | ', md5(pg_get_functiondef(p.oid)), 'secdef=' || p.prosecdef::text,
                   ...
                   case when p.proacl is null then 'default'
                        else coalesce((select string_agg(a::text, ',' order by a::text) from unnest(p.proacl) a), '') end)
  ...
  union all
  select 'event_trigger', evtname::text, evtevent || ' ' || evtfoid::regproc::text || ' ' || evtenabled::text
  ...
  select 'default_acl', pg_get_userbyid(d.defaclrole) || ' ' || coalesce(dn.nspname, '*') || ' ' || d.defaclobjtype::text,
         coalesce((select string_agg(a::text, ',' order by a::text) from unnest(d.defaclacl) a), '')
...
select kind, count(*) as n,
       md5(string_agg(key || '=' || coalesce(def, ''), chr(10) order by key, def)) as digest
