 ...
 * 5px tall on `--meter-bg`, with a violet→cyan fill. The Calls usage card and
 * the Setup progress card each had their own 6px bar on `--surface-3` with a
 * flat single-hue fill before this existed.
 *
 ...
 * chrome gradient and stays where it is (it is decorative, `aria-hidden`, and
 * `material.test.ts` pins its string in `app-sidebar.tsx`).
 *
 * `fill` overrides the gradient for a STATUS reading (near/over cap, done):
 * status is never a gradient.
 */
export const METER_TRACK = "h-[5px] w-full overflow-hidden rounded-full bg-[var(--meter-bg)]";
