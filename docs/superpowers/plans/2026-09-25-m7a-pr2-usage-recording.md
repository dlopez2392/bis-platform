...
- A11. The same `identifier` under a DIFFERENT idempotency key (the only way BIS sends one twice: its key expired after 24 hours, or the account's customer id changed) is accepted and deduplicated, accepted and COUNTED AGAIN, or refused with a `StripeInvalidRequestError`. **Unproven; the spec's "Stripe dedupes by identifier" (section 4) is relied on NOWHERE in this plan until Task 10 observes it.** `FakeGateway` therefore assumes the costlier answer (a second event, Task 7). Task 10 records which of the three (or "unproven", if the summary does not settle in time) as a test annotation, and ANY outcome other than "deduplicated as expected" blocks PR-3 from creating the first billed account (Task 11 handoff).
  **Observed (2026-09-25, Stripe TEST mode, CI `e2e`, `usage-meter.spec.ts`): REFUSED** — `StripeInvalidRequestError`, message `An event already exists with identifier <id>.` What was observed is narrow: the SAME customer, a FRESH idempotency key, seconds after the first send. The two resends production would actually make, a key expired after 24 hours and a changed customer id, remain UNOBSERVED and are assumed to be refused alike. Stripe's docs (an external claim, not verified here) promise identifier uniqueness only "within a rolling period of at least 24 hours", so a resend after about a day might instead be ACCEPTED and counted twice: a known limit, with PR-4's nightly reconciliation as the backstop (the same condition Task 10's note on what the probe cannot tell apart sets). Within what was observed no second event is recorded, so nothing is double-counted; but as merged, `meterEventFailureKind` called the refusal "row", so a row refused this way would never be stamped and its account would be skipped on EVERY tick, silently holding back all its later usage. **Fixed on `fix/usage-report-already-exists`:** `meterEventFailureKind(e, sentIdentifier)` has a third class, `"duplicate"`: `.type` `StripeInvalidRequestError` AND the message matching `^an event already exists with identifier (.+?)\.?$` case-insensitively AND the captured identifier equal to the row id sent. It is matched on the MESSAGE because the observation recorded no error code and the installed stripe 22.6.2 types name none for meter events (the generic enum's `resource_already_exists` is not known to be what Stripe sends here: assumption, not relied on). A reworded message falls back to "row", the pre-fix behaviour. `usageReport` stamps a duplicate via `markUsageReported` (reported_at still means "Stripe has it") and counts it `alreadyAtStripe` (`alreadyStamped` when the stamp resolves false, `unstamped` when it throws; the next tick's identical refusal stamps it), never `failed`, and the account's other rows go on. `FakeGateway` now refuses a held identifier under a new key with that error, keyed on the identifier alone whatever customer the new event names (assumption, unobserved: Stripe holds identifiers per Stripe account, not per customer). This outcome (the e2e annotation `refused, handled: ...`) clears A11's PR-3 blocker once the fix is merged, on the same terms as `deduplicated as expected`: together with PR-4's reconciliation as the backstop (Task 11, Step 5).
...
- Modify `apps/web/src/lib/billing/fake-gateway.ts`: `reportMeterEvent` (a new key is a new event: A11 is not assumed), `meterEvents`, `failOn.error`.
  **Superseded (2026-09-25):** A11's "Observed" note (line 54) replaces this fake behaviour: the fake now REFUSES a held identifier under a new key. This passage and the later ones that say "a new key is a new event" / "A11 is not assumed" are the historical task text, left as written.
...
**Known limit (accepted, review 2026-09-25): `callerSpoke` ignores `pendingCallerTurn`.** It reads only transcribed caller turns, so a caller cut off before their words were transcribed (the call ended mid-utterance, no booked/lead/message outcome) is not billed. This UNDER-bills, never over-bills; a fix would bill on the pending turn's audio, which this PR does not attempt.
...
**Known limits (accepted, review 2026-09-25).** (1) The turn-1 rule: a conversation whose FIRST reply was empty (the visitor read the unavailable line) and whose LATER turns were answered never bills, because only turn 1 attempts the leg. Under-bills, never over-bills. (2) The Activity page counts a website chat when the conversation STARTS, while billing counts it when Sofía's first reply succeeds, so the two numbers can differ; PR-3's usage screens must read `usage_events`, never the Activity page's count, or a client's screen will disagree with its bill.
...
- Orchestrator: read `verify` and `e2e` for the head SHA; read BOTH annotations (`A9/A10` and `A11 same identifier, new idempotency key`). What clears PR-3: `deduplicated as expected` on `A9/A10`, and on `A11 same identifier, new idempotency key` either `deduplicated as expected` or `refused, handled: ... (classified duplicate → stamped)` (the spec's own classifier read Stripe's refusal as "duplicate", so the reporter stamps the row instead of wedging the account). `refused, handled` clears A11 on the same terms as `deduplicated as expected`: once `fix/usage-report-already-exists` is merged, and with PR-4's reconciliation as the backstop for the resend the probe cannot reproduce (a key expired after 24 hours, or a changed customer id; see Task 10's note on what the test cannot tell apart, and A11's "Observed" note). **Anything else is a PR-3 blocker (not a PR-2 one):** `unproven` (the summary did not settle within the wait: re-run the e2e job on the same SHA until it is proven either way), an UNHANDLED refusal (the spec's assertion that the refusal is a `StripeInvalidRequestError` classified "duplicate" fails, and the e2e job is red: Stripe reworded the message or changed the error class, so the reporter would again hold that account's usage back every tick), and `accepted but double-counted` (the reporter must stop resending a row under a new key, e.g. reconcile against Stripe before resending a row first sent over 24 hours ago). PR-3 does not create the first billed account until its blocker is resolved.
...
1. **PR-3: Checkout, webhooks, the Billing card and the client Billing page.** Binding from this plan: (a) `account_billing.created_at` is the usage reporting start (G12): create the row when the subscription exists, or move the floor to a dedicated column; (b) remove `countBilledAccountsByPlan`'s 1000-row cap (PR-1 final review); (c) the usage screens read `usage_events` month to date through `usage_events_account_occurred_idx` — never the Activity page's chat count, which counts a chat at its start while billing counts it at Sofía's first successful reply (Task 6, known limits); (d) PR-3 creates no billed account until Task 10's A9/A10 AND A11 annotations both read `deduplicated as expected`, or the follow-up for the outcome observed has landed (Task 11, Step 5); (e) if PR-3 ever changes an account's `stripe_customer_id`, it must first settle that account's unreported usage rows (report them under the OLD customer, or reconcile them by hand): a row resent under the new customer that Stripe refuses as a duplicate is stamped `alreadyAtStripe`, which leaves that usage at Stripe under the OLD customer, never billed to the new one. The report's duplicate log line (`usage-report.ts`, "Stripe already held usage row <id> (sent for customer <id>; ...)") names the customer the pass SENT for, not the one Stripe holds it under.
