...
Both record the same way (between the provider send and the `sent` status write, G11), so they are one reviewable change. The automation leg (Task 4) records elsewhere, for a different reason (G10), which is why it is its own task.
