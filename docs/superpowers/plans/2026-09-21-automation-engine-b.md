...
- **A COPY TEST'S SEGMENT ASSERTION MEASURES `withOptOut(body)`, AND CARRIES AN ACCENTED NAME** (added 2026-09-22 from Task 6's review; Tasks 8 and 10 would have cloned the violation). Measuring the composed body alone asserts a string no customer receives. COMPUTE YOUR OWN BUDGET: 160 minus your template minus 23 for " Reply STOP to opt out." is how many characters of brand name still bill one segment — do not reuse another recipe's number. (Task 6 found its own default left only 13 characters, shortened the template from 124 septets to 110, and now has 27.) And every sibling copy test carries an accented brand name with its reason stated (`review-request-copy.test.ts:42-48`: "Rio Grande Valley client names are heavily Hispanic; 'García' is the common case, not an edge case"); an accented name is UCS-2 and three segments, which is unavoidable, but a test that never measures one is claiming GSM-7 for a client base half of whom are not. Measure the disclosed text, pin the encoding and the segment count (never `chars`), and include `García Roofing`.
...
/** Rung two, 08:05 CDT (NOT 09:05 — that is five minutes AFTER the 09:00 tick, and it would still have PASSED, because a future stamp is on the same local day; the case would have been green on a fixture that cannot occur) Sept 23 — a DIFFERENT instant from FOLLOWUP on
...
8. **quiet hours hold**, exactly as Task 3's: no send, no stamp, one held row whose `heldUntil` is NOON local — NOT 08:00. For any BAND-GATED recipe an 08:00 heldUntil is unreachable: the morning band OPENS at 08:00 and the default quiet window CLOSES at 08:00, so such a row can never be inside the window. (Task 3's confirm pass has no band, which is why 08:00 worked there.) Use review-request's own 21:00->12:00 window. Applies to Tasks 8 and 10 if either is band-gated..
...
5. **quiet hours hold**: no send, no stamp, one held row at NOON local. **NOT 08:00** — this recipe is BAND-GATED, and the morning band OPENS at 08:00 while the default quiet window CLOSES at 08:00, so a band-gated row can never be inside the default window (Global Constraint, and the same correction Task 6 had to make). Use review-request's own 21:00->12:00 window.
...
>
> **2026-09-22 postscript — decided by danlo, applied in e5b01b9.** The first option: the `stageRequired` branch no longer depends on `enabled` — it fires on any empty stage regardless of the toggle. Copy changed accordingly to "Pick the stage to watch before saving." (`:6018` above quotes the old, pre-decision copy and is left as written.)
