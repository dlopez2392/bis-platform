...
- [x] **Step 3: Add CRM isolation tests**
...
- [x] **Step 1: Clerk dark theme (fixes the invisible org switcher)**
...
```
**Execution note (deviation):** Clerk v7 (`@clerk/react` 6.x types) renamed the appearance key
`baseTheme` → `theme`. `baseTheme` fails typecheck: *"'baseTheme' does not exist in type
`Appearance<Theme>`"*. Shipped `appearance={{ theme: dark }}`. Installed `@clerk/themes@2.4.57`.

- [x] **Step 2: Workspace layout**
...
- [x] **Step 4: Share SubmitButton + link accounts rows** (`../../submit-button` compiles as-is — no move needed)
...
**Verified:** build EXIT=0 (route `ƒ /dashboard/accounts/[accountId]/contacts` listed), `pnpm check` EXIT=0
(6 files / 18 tests passed), headless GET of a nonexistent-account contacts URL → **307** to the Clerk
handshake (auth guard precedes `notFound()`). Commit `0de034a`.
```bash
...
Executed as written, no deviations. `pnpm --filter web build` EXIT=0 with
`ƒ /dashboard/accounts/[accountId]/contacts/[contactId]` in the route table;
`pnpm check` EXIT=0 (6 files / 18 tests). Note: the db suite can flake once with
`agency row missing: JWT issued at future` — local clock skew vs Supabase, unrelated
to app code; a re-run passes.
...
Executed as written, no deviations. `pnpm --filter web build` EXIT=0 with
`ƒ /dashboard/accounts/[accountId]/pipeline` in the route table; `pnpm check` EXIT=0
(6 files / 18 tests). Commit `a90edac`.
...
**Verified:** `pnpm check` EXIT=0 (6 files / 18 tests passed); `pnpm --filter web build` EXIT=0
with both `ƒ /dashboard/accounts/[accountId]/pipeline` and
`ƒ /dashboard/accounts/[accountId]/settings` in the route table.
...
Commit done: `d334e4f` (executor scope = commit only, no deviations). **`git push` NOT run** —
deferred to the orchestrator after the human review gate.
