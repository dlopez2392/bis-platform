...
describe("cursor", () => {
  ...
  it.each([undefined, "", "not-a-cursor", "2026-09-09T12:00:00.000Z", "abc|def",
    "2026-09-09T12:00:00.000Z|not-a-uuid", "|", "a|b|c"])(
    "drops the unusable cursor %j", (raw) => {
      expect(parseCursor(raw as string | undefined)).toBeUndefined();
    });

  it("parseTimeCursor keeps the calls behaviour: a bare ISO instant, or nothing", () => {
...
export function parseCursor(raw: string | undefined): RowCursor | undefined {
  ...
  if (!ISO.test(at) || !UUID.test(id)) return undefined;
  if (Number.isNaN(Date.parse(at))) return undefined;
  return { at, id };
}

/** The timestamp-only form `calls` already ships. */
export function parseTimeCursor(raw: string | undefined): string | undefined {
  if (!raw || !ISO.test(raw) || Number.isNaN(Date.parse(raw))) return undefined;
  return raw;
}
```
