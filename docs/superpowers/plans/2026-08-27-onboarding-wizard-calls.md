...
- [ ] **Step 2:** Runbook: add a "Client onboarding (wizard)" section — the Setup page now walks steps 1–9; buy-number stays a Telnyx-dashboard step; document `TELNYX_PUBLIC_KEY` (where to find it; setting it turns on signature validation AND closes the GET diagnostic path — set it AFTER the first smoke) and `APP_ORIGIN` (set to `https://app.bis-rgv.com`; why: link/sender match). Update the old step-5/6/7 text to point at the wizard.
