...
- **The rules the reviews of Tasks 1, 2 and 5 earned, in the order they were learned — each one from a shipped test that could not fail. They bind every task below:**
...
- **Do not touch the cost cap's goodbye instruction** (`incoming/route.ts:337-340` after Guard 1 shipped; it was `:311-313` when this plan was written). Its open-ended wording is correct for a real conversation that runs long. Guard 1 gets its own constrained instruction; the two must not be "unified".
- **"A refused call writes NO `calls` row" is a load-bearing invariant of Guard 2, not an accepted side effect.** The self-disabling chain, in one sentence: `startCallRow` inserts no outcome → `calls.outcome` defaults to `abandoned` (`0019_voice_core.sql:53`) → `countCallerHistorySince`'s "other" half is `.neq("outcome","spam")`, so that row counts as `otherCalls` → `decideReputation`'s `otherCalls > 0` clause clears the caller — meaning **the act of blocking a caller would clear their block**, and Guard 2 would fire exactly once per number and then never again, in silence. So: surfacing declines to an operator goes through a log line, a metric, or a NEW column — **never** through `startCallRow` — unless `countCallerHistorySince`'s "other" query changes in the same commit. Pinned by `expect(startCallRowMock).not.toHaveBeenCalled()` on both decline tests in `incoming/route.test.ts`; a reviewer who hoisted `startCallRow` above the gates got 30/30 green before those existed.
- **Every test in `incoming/route.test.ts` must set `unwrapMock`, and must assert something positive.** `beforeEach` resets it, so an unset `unwrapMock` resolves `undefined`, reading `event.type` throws, and the route's outer catch returns a bare `{ ok: true }` — which satisfies `expect(json.declined).toBeUndefined()` against a route with the feature **entirely removed**. That is what made one brief's own test bodies vacuous. Pair every decline/accept assertion with a positive one (`fetchMock` called once, or the exact `declined` value).
