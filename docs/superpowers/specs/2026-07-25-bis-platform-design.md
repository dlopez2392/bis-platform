...
### 8a. Roadmap status (as of 2026-09-21, automation engine part C, #108)
...
| M3 | ⚠️ partial | the engine's shared pieces shipped 2026-09-21 (part C, #108): one `automation_log`, one quiet-hours window per account (deferred, never skipped, released by a queue), usage in units and a client-visible history at /activity. Part B shipped 2026-09-22 (#TBD): four more recipes — appointment confirmations with a YES/NO reply recorded on the booking, referral asks as the completed-job ladder's third rung, a once-ever reactivation email to a past customer, and a pipeline-driven quote follow-up. Still owed: a rule builder, deferred until a second client's needs diverge from the catalogue. | more recipes (part B, its own spec) and a rule builder, deferred until a second client's needs diverge from the catalogue. |
