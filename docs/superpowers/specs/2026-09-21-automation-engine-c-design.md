...
**The weekly reports are exempt from the window** (they go to the owner and the agency on Monday morning in the account's zone). The client weekly report writes one `email` row per account per week, source `weekly_report`, so "what went out" is complete for it. The agency roll-up logs nothing: `automation_log.account_id` is `NOT NULL` (migration 0046's `source` CHECK constraint also refuses `weekly_agency_report`, struck by amendment 9), and the roll-up covers every account, not one — there is no single account id to log it under.
