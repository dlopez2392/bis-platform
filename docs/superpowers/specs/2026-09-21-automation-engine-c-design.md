...
**Amended — the reminder exemption, named from the tree.** It bites the SMS reminder (due ~2h before the appointment), almost never the email reminder (due ~24h before). The rule, applied by `holdOrSend` for the two reminder sources: **a reminder whose appointment starts before the current window ends sends now** (the 6:45 AM text for a 7:30 AM job). At release time, a held reminder whose appointment has already started is flipped to `skipped` with reason "Appointment already started".
