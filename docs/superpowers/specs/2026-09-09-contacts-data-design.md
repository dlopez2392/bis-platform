...
**One migration, 0030 (added 2026-09-09 after danlo chose server-side sorting).** The spec originally promised none, and that held until sorting moved to the server: the UI sorts by `contactDisplayName`, a JS expression, which Postgres cannot order by. `0030_contacts_sort_name` adds a STORED GENERATED column plus an index It also carries a column-level `grant select (sort_name) … to authenticated` which, on inspection AFTER applying, turned out to be a **no-op**: `public.contacts` has TABLE-level grants that already cover any new column. The mistake came from reading `information_schema.column_privileges`, which expands a table-level grant into one row per column — filtered to one role and one privilege it is indistinguishable from a real per-column grant. Access control on this table is RLS. Other tables here (calendars) do use genuine column-level grants, so the check is worth doing; it just has to be done per table. Everything else this needs already exists on `contacts`:
