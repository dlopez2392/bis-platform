...
Keep the notes from Part A until Part F is done, because they are what every
rollback below restores.
...
the development instance, `https://topical-redfish-40.clerk.accounts.dev`. The
second was kept on 2026-09-14 so e2e could stay on the development instance
(`clerk-setup.md`, Part E). Supabase accepts a session token from either one.
...
(`packages/db/src/test/user-client.integration.test.ts:7`), and Clerk's Backend
API mints a session token for any user with nothing but the secret key: create
a session, then a token (the same file, lines 36-62, does exactly this against
the CI project). So the development secret key alone reads and writes every
tenant's production data. That key is held by the repository secret
`CLERK_SECRET_KEY` (read by `ci.yml` and `screenshots.yml`), by Vercel Preview,
and by local `apps/web/.env.local` files.
...
project (documented in `.env.example`'s Stripe block; this runbook did not
read the values), while `clerk-setup.md` Part G keeps Preview on the
development Clerk pair — which is why Preview needs path 1. Vercel builds a preview
...
| Local `pnpm dev` while `apps/web/.env.local` still names production (plan step D7) | development | production | **Yes** |
...
a separate change) and local `pnpm dev` against production (fixed by D7,
...
`clerk-setup.md` Part E followed for this very entry. D is the step that closes
path 1; B and C close path 2. Rotation comes last on purpose: a production key
...
2. **Vercel → Environment Variables, Preview only.** Where a row is shared with
   ...
   a Preview-only row. Vercel silently ignores "Add New" on a name that already
   exists for that environment, so remove first, then add.

   | Variable | Preview value | Type |
   |---|---|---|
   | `NEXT_PUBLIC_SUPABASE_URL` | `https://odnobiodsftffphuuosz.supabase.co` | Config (a `NEXT_PUBLIC_` value cannot be Sensitive) |
...
4. **Build a new preview** (push any branch, or Redeploy the latest preview
   with the build cache off). `NEXT_PUBLIC_` values are compiled in at build
   time, so an old preview keeps the old database.
...
- Sign in on the new preview and open `/dashboard/accounts`. It lists the CI
  ...
  row.)
- In Vercel, `SUPABASE_DB_URL` has no row with Preview ticked, and the three
  Supabase rows that do are Preview only.
...
**Roll back.** Re-tick Preview on the production rows (or restore the old
Preview rows from the A1 notes) and build a new preview. This re-opens path 2,
so treat it as a stopgap.
...
1. **Vercel → bis-platform → Settings → Deployment Protection → Vercel
   ...
   - If the dashboard offers only **Standard Protection**
     ...
     cron and webhook lines below. [assumption: scope names as Vercel's API
     documentation listed them on 2026-09-26]
...
**Roll back.** Switch Vercel Authentication off. One toggle, no redeploy.
...
Accepted: `screenshots.yml` and local `pnpm dev` against production fail the
same way until they move (the table above).
...
| After Part D | **401** | **200** |

If the preview tab answers 401 BEFORE the change, the check is broken (most
likely the `apikey`), not the path closed. Fix the check first. The production
tab's 200 after the change is what proves the 401 is about the issuer and not
the request.
...
1. The check table's "After" row: 401 in the preview tab, 200 in the production
   tab.
2. Production still works: sign in at `app.bis-rgv.com`, the agency dashboard
   lists accounts, and an in-account list returns rows (`clerk-setup.md`
   Part I, step 3). The smoke `curl` from Part C is unchanged.
...
| `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET` | Resend → API Keys; the webhook's signing secret | Vercel Production |
...
| `DAILY_API_KEY` | Daily → Developers | Vercel Production |
...
- **The Supabase secret key.** Supabase lists secret keys by name with a
  masked prefix; match the first characters Vercel reveals for Preview's value
  against A3's list. If Preview had a key of its own, delete that key and you
  are done. If it held Production's, create a new key, switch Production to it
  (Remove, then add, Sensitive), redeploy, verify the smoke and one cron run,
  then delete the old key. [assumption: a `sb_secret_` key can be deleted
  without touching the others; a legacy `eyJ…` service-role key cannot be
  rotated that way, because rotating the JWT secret also changes the anon key.
  If Preview held a legacy key, move Production to a `sb_secret_` key first,
  then disable the legacy keys.]
- **Forms.** While `FORM_TOKEN_SECRET` is unset, the public-form render token is
  signed with a key derived from the Supabase secret key
  (`apps/web/src/lib/forms/guards.ts:36-42`), so rotating either one fails
  the render-token check for a form opened before the redeploy and submitted
  after it (tokens live 30 minutes, `guards.ts:19`). Rotate outside business
  hours.

**Verify** each rotation where it is used: the production smoke, one cron run
returning 200, one form submission, one test call or text if a voice or SMS
credential moved. Then confirm the old value is gone from the provider's list.
...
2. Put it in: the repository secret `CLERK_SECRET_KEY` (Remove, then add; paste
   ...
   `CLERK_SECRET_KEY` (Preview only), and local `apps/web/.env.local`.
...
| Non-secret configuration, fine on Preview | `APP_ORIGIN`, `EMAIL_FROM`, `AGENCY_SUPPORT_EMAIL`, `VERCEL_TEAM_ID`, `VOICE_OPENAI_PROJECT_ID`, `TELNYX_PUBLIC_KEY`, `TELNYX_VOICE_CONNECTION_ID`, `SOFIA_WEB_ORIGINS`, `SOFIA_WEB_NUMBER`, `SOFIA_WEB_DISPLAY_NUMBER`, `REALTIME_MODEL`, every `PHONE_*` knob |
...
| Leave unset on Preview | `EMAIL_DEV_REDIRECT_TO`, `SMS_DEV_REDIRECT_TO`, `VOICE_FORWARD_TO`: inert without the provider keys, and nothing on a preview should send |
