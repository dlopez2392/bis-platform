...
- `apps/web/src/lib/auth.ts`, `apps/web/src/lib/db.ts`, `apps/web/src/lib/account-route.ts`, `apps/web/src/middleware.ts`
...
- **Middleware** is `clerkMiddleware` protecting `/dashboard(.*)`; the matcher excludes `_next` and static files and includes `/api`. Public routes (`/b`, `/f`, `/embed.js`, webhooks, cron, voice) authenticate themselves (tokens, signatures, secrets), never through Clerk.
